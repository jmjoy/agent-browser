...
By default each session binds its WebSocket stream server to `127.0.0.1` on an OS-assigned port. To control the bind address or port, set `AGENT_BROWSER_STREAM_ADDR` and `AGENT_BROWSER_STREAM_PORT`:
