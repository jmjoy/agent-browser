...
agent-browser stream status           # Show runtime streaming state, bind address, and bound port
...
```bash
agent-browser stream enable           # Start runtime WebSocket streaming on 127.0.0.1 with an auto-selected port
agent-browser stream enable --addr 0.0.0.0 --port 9223  # Bind a specific address and port
agent-browser stream status           # Show enabled state, bind address, port, browser connection, screencasting
agent-browser stream disable          # Stop runtime streaming and remove the .stream metadata file
```

Streaming is enabled automatically for all sessions. Use these commands to check status, re-enable on a specific address and port, or disable streaming.
